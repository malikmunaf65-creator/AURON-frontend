...
// ─── helper: FileReader wrapped in a Promise ───────────────────────────────
...
export default function UploadView({ onNavigate, onAddHistory, userProfile, appLanguage = "English (US)" }: UploadViewProps) {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setTimeout(() => {
          ...
          setIsUploading(false); // ← simulation done, unlock the button
        }, 800);
      } else {
  ...
  // ─── FIXED: pure async/await, no nested callbacks ─────────────────────────
  const handleInitiateAnalysis = async () => {
    ...
    try {
      // 1. Read file — await the Promise wrapper instead of callback hell
      const base64Payload = await readFileAsBase64(selectedFile);
 
      // 2. Send to backend with timeout
      ...
      const durationSimulation = parseFloat((Math.random() * 3 + 1.5).toFixed(1));
      ...
      // ← ALWAYS runs, no matter what — this was the bug
          ...
          >
            ...
            <div className="text-center z-10 px-4">
              ...
              <p className="font-sans text-xs text-[#c7c4d8]/60">
                {t("or", appLanguage)}{" "}
                <span className="text-[#92dbff] group-hover:underline cursor-pointer">
                  {t("browse files", appLanguage)}
                </span>
              </p>
            </div>
        ...
        {selectedFile && (
          <div className="flex flex-col gap-3">
            ...
            <button
              onClick={handleClear}
              onMouseEnter={playHoverSfx}
              className="w-full bg-white/5 border border-white/10 text-[#c7c4d8] hover:text-white font-mono text-[10px] uppercase py-2.5 rounded hover:bg-white/10 transition-colors cursor-pointer"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#05050a]/90 backdrop-blur-xl animate-fadeIn">
          <div className="w-full max-w-md bg-[#1f1f26]/85 border border-red-500/15 shadow-[0_0_50px_rgba(239,68,68,0.15)] rounded-2xl p-6 md:p-8 relative overflow-hidden backdrop-blur-md">
            ...
            <div className="flex flex-col gap-4 text-center">
              ...
              <div>
                <h3 className="font-sans text-lg md:text-xl font-bold text-white tracking-wide">Analysis Matrix Locked</h3>
              ...
              <p className="font-sans text-xs text-[#c7c4d8]/75 leading-relaxed">
                Guest nodes are restricted to exactly <strong className="text-white">one (1) free</strong> audio upload analysis transaction.
              </p>
              <div className="flex flex-col gap-2 mt-2">
                <button
                  onClick={() => { playClickSfx(); setShowLimitModal(false); onNavigate("signup"); }}
                  onMouseEnter={playHoverSfx}
                  className="w-full bg-[#4f44e2] hover:bg-[#4f44e2]/90 text-white font-mono text-xs font-bold uppercase py-3.5 rounded-xl flex items-center justify-center gap-2 transition duration-200 cursor-pointer"
                >
                ...
                <button
                  onClick={() => { playClickSfx(); setShowLimitModal(false); }}
                  onMouseEnter={playHoverSfx}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#05050a]/90 backdrop-blur-xl animate-fadeIn">
          <div className="w-full max-w-md bg-[#1f1f26]/90 border border-[#92dbff]/30 shadow-[0_0_50px_rgba(146,219,255,0.15)] rounded-2xl p-6 md:p-8 relative overflow-hidden backdrop-blur-md">
            ...
            <div className="flex flex-col gap-4 text-center">
              ...
              <div className="flex flex-col gap-2 mt-2">
                <button
                  onClick={() => { playClickSfx(); setShowReadyModal(false); onNavigate("signup"); }}
                  onMouseEnter={playHoverSfx}
                  className="w-full bg-[#4f44e2] hover:bg-[#4f44e2]/90 text-white font-mono text-xs font-bold uppercase py-3.5 rounded-xl flex items-center justify-center gap-2 transition duration-200 cursor-pointer"
                >
                  <KeyRound className="w-4 h-4 text-[#92dbff]" />
                ...
                <button
                  onClick={() => { playClickSfx(); setShowReadyModal(false); onNavigate("login"); }}
                  onMouseEnter={playHoverSfx}
                  className="w-full bg-white/5 hover:bg-white/10 text-white/70 hover:text-white font-mono text-xs uppercase py-3 rounded-lg transition duration-200 border border-white/10 cursor-pointer"
                >
                  {t("Sign-In Existing Agent", appLanguage)}
                </button>
                <button
                  onClick={() => { playClickSfx(); setShowReadyModal(false); setSelectedFile(null); setUploadProgress(0); setStatusMessage(""); }}
                  onMouseEnter={playHoverSfx}
